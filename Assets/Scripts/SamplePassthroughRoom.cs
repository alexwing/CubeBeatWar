    ...
    void InitializeRoom()
    {
        ...
        if (sceneAnchors != null)
        {
            for (int i = 0; i < sceneAnchors.Length; i++)
            {
                ...
                {
                    // move the world slightly above the ceiling, so the virtual ceiling doesn't Z-fight
                    if (_envRoot)
                    {
                        Vector3 envPos = _envRoot.transform.position;
                        float ceilingHeight = instance.transform.position.y + _groundDelta;
                        _envRoot.transform.position = new Vector3(envPos.x, ceilingHeight, envPos.z);
                    }
                }
